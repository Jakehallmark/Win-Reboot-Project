...
# Redirect macOS users to the macOS-specific script
if [[ "$(uname -s)" == "Darwin" ]]; then
  MACOS_SCRIPT="$SCRIPT_DIR/macos-setup.sh"
  if [[ -f "$MACOS_SCRIPT" ]]; then
    exec bash "$MACOS_SCRIPT" "$@"
  else
    echo "[+] macOS detected. Downloading macos-setup.sh..."
    curl -fsSL https://raw.githubusercontent.com/Jakehallmark/Win-Reboot-Project/main/macos-setup.sh | bash
    exit $?
  fi
fi
